...
import Sectorsdesciption from "./inc/description";
...
function Occupation() {
  ...
  const [occupationDescription, setOcupationDescription] = useState(null);
  ...
  const filterKeyFactsByOccupation = (e) => {
    ...
    const keyFacts = filteredKeyFacts.filter(
      (item) => item.attributes.occupation?.data?.id === selected
    );
  ...
  const changeOccupationDescription = ({
    ...
    setOcupationDescription(description);
  };
